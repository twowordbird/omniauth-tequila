  ...
  describe 'GET /auth/tequila' do
    ...
    shared_examples 'Tequila createrequest' do
      describe 'createrequest body' do
        ...
        it { should match( /^service=DISAL GitLab$/ ) }
        it { should match( /^require=group=disal-git$/ ) }
  ...
  describe 'GET /auth/tequila/callback' do
    ...
    context 'when Tequila server works' do
      ...
      describe 'omniauth.auth' do
        ...
        its(:uid) { should == '183256' }
      end

      describe 'omniauth.auth.info' do
        subject { last_request.env['omniauth.auth']['info'] }
        it { should have(5).items }
        its(:name)       { should == 'William Christopher Evans' }
        its(:first_name) { should == 'William Christopher' }
        its(:last_name)  { should == 'Evans' }
        its(:email)      { should == 'william.evans@epfl.ch' }
        its(:phone)      { should == '+41 21 6935797' }
      end
      ...
      describe 'omniauth.auth.extra' do
        ...
        its(:provider)     { should == '' }
        its(:specrequire)  { should == 'group=disal-git' }
        its(:status)       { should == 'ok' }
        its(:speckey)      { should == 'esu3r5e6fy0c616af80y5ienzrj2n6x8' }
        its(:group)        { should == 'disal-git' }
        its(:requesthost)  { should == '128.178.90.148' }
        its(:authstrength) { should == '1' }
        its(:org)          { should == 'EPFL' }
        its(:host)         { should == '128.178.195.240' }
